...
(defparameter *trace-to-file* nil "if T, pipes the ACT-R trace to a provided text file")
(defparameter *calculate-acc* nil "if T, calculates response accuracies in reading tasks") ;; pmaetzig added; will replace *trace-to-file* route for acc calculation in the future
...
(defvar *response-acc* nil "accuracy of a sentence's dependency")  ;;[pmaetzig] 2017-08-17: added
(defvar *dependency-list* nil "a variable to hold the list of (word1 dependency word2) for later response accuracy calculation")
(defvar *correct-dependency* nil "a variable to store the correct dependency for a given sentence")
(defvar *correct-dependency-list* nil "a variable to store the correct dependencies for all sentence types, provided in dependencies.txt")
...
(defvar *accuracy-data* nil "Table of response accuracies")
...
(defvar *tracefile* (concatenate 'string *output-dir* "/trace.txt") "the file to which the ACT-R trace will be written")
...
;;; [pmaetzig] 2017-08-18: function for calculating response accuracy
(defun calculate-accuracy (dep-list correct-dep)
  "Calculates response accuracy by searching for correct-dep (a list of strings) in
   dep-list (a list of lists of strings."
  (let ((acc 0))
    (loop for dep in dep-list
        if (equalp dep correct-dep) 
        do (return (+ 1 acc)))))

(defun accuracy-table (acc)
  "Writes response accuracy and metadata into table format."
  (let ((table nil))
    (push-last (list *experiment* *simulation* *item* acc)
               table)
    table))
...
(defun string-split (string)
  "Returns a list of substrings of string divided by ONE space each.
   ...
   Copied from the Common Lisp cookbook."
   (loop for i = 0 then (1+ j)
     as j = (position #\Space string :start i)
     collect (subseq string i j)
     while j))

(defun read-dependency-file (&optional (dep-file "dependencies.txt"))
  "Reads the dependency file (by default 'dependencies.txt' in the current directory)
   and returns a list of (condition, dependency) lists. Used in the runtime calculation
   of response accuracies."
  (with-open-file (stream dep-file)
    (loop for line = (read-line stream nil nil)
          while line
          collect line)))

(defun dep-string-to-list (dep-list)
  "documentation here"
  (loop for str in dep-list
        collect (string-split str)))
  
(defun find-correct-dependency (correct-dep-list sentence-type)
  "Loops through list of correct dependencies for all sentence types,
   and returns the correct dependency for the current sentence."
  (loop for dep in correct-dep-list
        do (when (equalp (first dep) (string sentence-type))
             (return (cdr dep)))))
...
;; at the moment trace-to-file always calls present-whole-sentence with
;; (performance T), this could be changed in the future
(defun trace-to-file (sentence filename &optional (params nil))
  (with-open-file (*standard-output* filename :direction :output  :if-exists :append :if-does-not-exist :create)
  ;(with-open-file (*standard-output* filename :direction :output)
      (present-whole-sentence sentence *max-time* params t)))
...
(defun delete-output ()
  ...
  (let ((file (first (directory (concatenate 'string *output-dir* "/subjects.txt")))))
    (if file (delete-file file)))
  ;(let ((file (first (directory (concatenate 'string *output-dir* "/trace.txt")))))
    ;(if file (delete-file file)))
  )
