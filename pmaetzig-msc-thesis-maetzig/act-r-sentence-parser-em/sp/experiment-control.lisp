...
(defun run-paramset-em (name &optional (iterations 1) (params nil))
  ...
  (let* ((location (concatenate 'string *output-dir* "/../paramsearch/" (string *experiment*) "/"))
         ...
         (*tracefile* (concatenate 'string location (write-to-string *paramset*) "-trace.txt"))
         (accfile (concatenate 'string location (write-to-string *paramset*) "-accuracies.txt"))
         (successes 0))
...
;;  (write-table *attachment-data* attachfile)
  (write-table *accuracy-data* accfile)
...
(defun write-headers nil
  ...
  ;(table-header "experiment simulation item acc" *accuracy-file*)  ;;[pmaetzig] 2017-08-17: added
  )
...
(defun write-data nil
  ...
  ;(write-table *accuracy-data* *accuracy-file*)  ;;[pmaetzig] 2017-08-17: added
  ...
  (let ((conditions (first (get '*experiments* name)))
    ...
    (dolist (c conditions)
      (let* ((cname (pop c))
             ...
             ;(regions c)
             (success nil)
             (condition-acc 0))  ;;[pmaetzig] 2017-08-16
             
        (setf *item* cname)
        
        ;; The current correct dependency and the dependency list are reset before
        ;; a new sentence is read. *dependency-list* is updated during the 
        ;; present-whole sentence call below.
        (setf *correct-dependency* nil)
        (setf *dependency-list* nil)
        ;;; --------------------------------------------------------- ;;;
        (if *trace-to-file*
          (setf success (trace-to-file sent *tracefile* params))
        ...
        ;;; --------------------------------------------------------- ;;;
        ;;; [pmaetzig] 2017-08-18: CALCULATING RESPONSE ACCURACY ---- ;;;
        ;;; --------------------------------------------------------- ;;;
        (when *calculate-acc*
          (setf *correct-dependency* (find-correct-dependency *correct-dependency-list* cname))
          (setf condition-acc (calculate-accuracy *dependency-list* *correct-dependency*))
          (setf *accuracy-data* (append *accuracy-data* (accuracy-table condition-acc)))
          ;; (format t "Current dependency list: ~A~% Current correct dependency: ~A" *dependency-list* *correct-dependency*)
          ;; (print *accuracy-data*)
         )
        ...
        ;; store response accuracies  ;;[pmaetzig] 2017-08-17: added
        ;(setf *accuracy-data* (append *accuracy-data* (accuracy-table *accuracies*)))
        ;;;
        (when *record-visloc-activations*
...
(defmacro search-param-space-em (experiment iterations pspace &key (script T))
  ...
  (setf *correct-dependency-list* (dep-string-to-list (read-dependency-file)))
  (delete-output)
...
; (parameters '(list :v nil)))
  ...
  (setf code (push-last code do-code))
    ...
    `(progn 
       ,code
       ;; [pmaetzig] 2017-08-20: Disabled outputs for only recording response accuracies.
       (setprint off)
       (setf *VERBOSE* nil)
       (setf *show-window* nil)
       (setf *record-times* nil)
