...
"+++  Relation: ~s is ~s of ~s  +++" 
  ...
  ;;[pmaetzig] 2017-08-16: stores the dependency in a list and stores it in a global variable
  (when *calculate-acc*
    ;(setf *dependency-list* nil)
    (if (not (equal *dependency-list* nil)) ;; per default NIL
      (push-last (list dependent relation head) *dependency-list*) 
      (setf *dependency-list* (list (list dependent relation head)))
    )
  )
  )
